...
export class ErrorBoundary extends Component<Props, State> {
  ...
  render() {
    ...
    if (this.state.hasError) {
      ...
      return (
        <div className="min-h-[400px] flex flex-col items-center justify-center p-8 bg-card">
          <div className="max-w-md w-full bg-card border border-border shadow-sm p-8">
            <div className="flex items-center gap-3 mb-4">
              <AlertTriangle className="w-8 h-8 text-red-600" />
              <h2 className="text-2xl font-bold">{strings.title}</h2>
            </div>

            <p className="text-muted-foreground mb-4 text-sm">{strings.description}</p>
            ...
            <div className="flex gap-3">
              <Button
                ...
                variant="outline"
                className="flex-1 border-border rounded-lg shadow-sm hover:translate-y-[1px] hover:translate-x-[1px] hover:shadow-none transition-all"
              >
                {strings.tryAgain}
              </Button>
              <Button
                onClick={this.handleReload}
                className="flex-1 bg-primary hover:bg-blue-800 text-white rounded-lg border border-border shadow-sm hover:translate-y-[1px] hover:translate-x-[1px] hover:shadow-none transition-all"
              >
