...
export function DiffPreviewModal({
  ...
  if (!diffSummary || !detailedChanges) {
    return (
      ...
      >
        <DialogContent className="max-w-5xl max-h-[90vh] overflow-hidden flex flex-col p-6 bg-card border border-border shadow-sm">
          ...
          <div className="mt-6 border border-border bg-card p-4 text-xs text-foreground">
      ...
      <DialogContent className="max-w-5xl max-h-[90vh] overflow-hidden flex flex-col p-6 bg-card border border-border shadow-sm">
        ...
        <div className="border border-border bg-card p-4 mt-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="text-sm font-bold">{t('tailor.diffModal.summary')}</h3>
          </div>
          ...
          {diffSummary.high_risk_changes > 0 && (
            <div className="mt-4 border border-[#F97316] bg-[#FFF7ED] p-3 flex items-start gap-3">
        ...
        {errorMessage && (
          <div className="mt-4 border border-red-600 bg-red-50 p-3 text-xs text-red-700">
            {errorMessage}
          </div>
        )}
        ...
        <div className="flex justify-between items-center pt-4 border-t-2 border-border bg-card -mx-6 -mb-6 px-6 py-4">
          ...
          <Button onClick={onConfirm} className="gap-2 bg-green-500 hover:bg-green-600">
...
function StatCard({ label, value, variant }: StatCardProps) {
  ...
  return (
    <div className={`border p-3 ${colors[variant]}`}>
...
function ChangeItem({ change }: ChangeItemProps) {
  const typeColors = {
    ...
    modified: 'border-l-4 border-primary bg-primary/10',
  };
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1">
          {change.original_value && (
            <div className="line-through text-[#DC2626] text-sm mb-1">{change.original_value}</div>
          )}
          {change.new_value && <div className="text-foreground text-sm">{change.new_value}</div>}
        </div>
