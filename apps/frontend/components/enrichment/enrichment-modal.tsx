  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 flex items-center justify-center p-5 sm:p-10">
        <div className="relative w-full h-full max-w-[1200px] bg-card border border-border shadow-sm flex flex-col overflow-hidden">
          {/* Header */}
          <div className="flex items-center justify-between px-6 py-4 border-b-2 border-border bg-muted/50">
            <div className="flex items-center gap-3">
              <Sparkles className="w-5 h-5" />
              <h1 className="text-lg font-bold">{t('enrichment.title')}</h1>
            </div>
            {/* Only show close button in non-loading states */}
            {!['analyzing', 'generating', 'applying'].includes(state.step) && (
              <button onClick={handleClose} className="p-1 hover:bg-muted transition-colors">
