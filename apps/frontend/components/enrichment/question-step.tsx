...
export function QuestionStep({
  ...
  // Track the question ID to reset local answer when question changes
  const [trackedQuestionId, setTrackedQuestionId] = useState(question.question_id);
  const [localAnswer, setLocalAnswer] = useState(answer);

  // Reset local answer when question changes (derived state pattern)
  if (trackedQuestionId !== question.question_id) {
    setTrackedQuestionId(question.question_id);
  ...
  return (
    <div className="flex flex-col h-full min-h-[500px]">
          ...
          {Array.from({ length: totalQuestions }).map((_, i) => (
            <div
              ...
              className={`h-1.5 w-6 transition-colors ${
                i < questionNumber ? 'bg-black' : i === questionNumber - 1 ? 'bg-black' : 'bg-muted'
              }`}
            />
          ))}
        </div>
      ...
      {item && (
        <div className="mb-6">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-accent border border-border text-sm">
      ...
      <div className="flex-1">
        ...
        <p className="text-xs text-muted-foreground/60 mt-2">{t('enrichment.shortcutHint')}</p>
      </div>
