  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex items-center gap-2 p-4 border-b border-border bg-muted/50">
        ...
        <h3 className="text-sm font-bold text-foreground">{t('builder.jdMatch.yourResume')}</h3>
  ...
  return (
    <span>
      {segments.map((segment, i) =>
        segment.isMatch ? (
          <mark key={i} className="bg-yellow-200 text-foreground px-0.5">
            {segment.text}
          </mark>
        ) : (
  ...
  return (
    <span
      className={`inline-block px-2 py-0.5 text-xs ${
        isMatch ? 'bg-yellow-200 text-foreground font-medium' : 'bg-card text-muted-foreground'
      }`}
    >
      {text}
    </span>
