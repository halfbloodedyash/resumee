...
export const RegenerateDiffPreview: React.FC<RegenerateDiffPreviewProps> = ({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[800px] max-h-[90vh] p-0 gap-0 rounded-lg overflow-hidden">
        ...
        <div className="px-6 pt-4">
          <div className="inline-flex items-center gap-2 px-3 py-1 bg-green-50 border border-green-200 text-green-700 text-xs">
            ...
            <div className="border border-border bg-[#FFF9DB] px-4 py-3">
              ...
              <ul className="mt-2 space-y-1">
                {regenerateErrors.map((failed) => (
                  <li key={failed.item_id} className="text-xs text-foreground">
                    • {getItemLabel(failed)}
                  </li>
                ))}
              </ul>
            ...
            <div key={item.item_id} className="border border-border">
              ...
              {expandedItems.has(item.item_id) && (
                <div className="border-t border-border">
                  ...
                  <div className="p-4">
                    <div className="text-xs text-muted-foreground mb-2 flex items-center gap-2">
                      <span className="w-3 h-3 bg-green-500 border border-border" />
        ...
        <DialogFooter className="p-4 bg-accent border-t border-border flex-row justify-between gap-3">
          <Button
            variant="outline"
            onClick={onReject}
            disabled={isApplying}
          ...
          <Button variant="success" onClick={onAccept} disabled={isApplying} className="rounded-lg">
            {isApplying ? (
              <>
