  ...
  return (
    <div className="border border-border bg-card rounded-xl shadow-sm">
      ...
      <button
        ...
        className="w-full flex items-center justify-between p-3 hover:bg-accent/50 transition-colors rounded-xl"
      >
                ...
                >
                  ...
                  <span
                    className={`text-[9px] font-medium ${
                      settings.template === template.id ? 'text-primary' : 'text-muted-foreground'
                    }`}
                  >
              ...
              <div className="flex gap-2">
                {(Object.keys(ACCENT_COLOR_MAP) as AccentColor[]).map((color) => (
                  <button
                    ...
                    className={`flex items-center gap-2 px-3 py-2 border rounded-lg text-xs transition-all ${
            ...
            <div className="flex gap-2">
              {(Object.keys(PAGE_SIZE_INFO) as PageSize[]).map((size) => (
                <button
                  ...
                  className={`flex-1 px-3 py-2 border rounded-lg text-xs transition-all ${
          ...
          <div>
            ...
            <div className="space-y-3">
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                ...
                >
                  <span
                    className={`absolute top-0.5 w-3.5 h-3.5 bg-card rounded-full shadow-sm transition-all ${
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                <button
                  ...
                  className={`relative w-10 h-5 rounded-full transition-all ${
                    settings.showContactIcons ? 'bg-primary' : 'bg-muted'
                  }`}
                >
                  <span
                    className={`absolute top-0.5 w-3.5 h-3.5 bg-card rounded-full shadow-sm transition-all ${
                      settings.showContactIcons ? 'left-5' : 'left-0.5'
                    }`}
                  />
                </button>
                <span className="text-xs text-foreground">
...
const MarginSlider: React.FC<MarginSliderProps> = ({ label, value, onChange }) => {
  return (
    <div className="flex items-center gap-2">
      ...
      />
      <span className="text-xs w-6 text-right text-foreground">{value}</span>
    </div>
  );
};
      ...
      <div className="flex gap-1">
        {levels.map((level) => (
          <button
            ...
            className={`w-6 h-6 text-xs border rounded-md transition-all ${
              value === level
                ? 'bg-primary text-primary-foreground border-primary shadow-sm'
                : 'bg-card text-muted-foreground border-border hover:border-foreground/30'
            }`}
