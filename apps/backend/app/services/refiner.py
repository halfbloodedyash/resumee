...
def _extract_all_text_impl(data: dict[str, Any]) -> str:
    """Implementation of text extraction (no caching).

    The previous lru_cache-based approach used serialized JSON as cache keys,
    which consumed significant memory for minimal benefit (resumes are rarely
    re-extracted during a single request). Removed to prevent memory leaks.
    """
    parts: list[str] = []
