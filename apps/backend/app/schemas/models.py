...
def normalize_resume_data(data: dict[str, Any]) -> dict[str, Any]:
    ...
    Returns a shallow copy so the original database record is never mutated.
    """
    # Work on a shallow copy so we don't mutate the TinyDB cached dict
    result = dict(data)
    if not result.get("sectionMeta"):
        # Use deepcopy to avoid shared mutable reference bug
        # Without this, all resumes would share the same list reference
        result["sectionMeta"] = copy.deepcopy(DEFAULT_SECTION_META)
