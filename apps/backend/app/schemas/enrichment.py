...
class AnalysisItemContext(BaseModel):
    """Compact representation of an analysis item for the enhance request."""
...
class AnalysisQuestionContext(BaseModel):
    """Compact representation of a question for the enhance request."""
...
class EnhanceRequest(BaseModel):
    ...
    Now carries the original analysis context so the backend does NOT need
    to re-run the LLM analysis (which would produce non-deterministic
    question/item IDs and silently map answers to the wrong items).
    """

    resume_id: str
    answers: list[AnswerInput]
    # Analysis context forwarded from the /analyze response
    items_to_enrich: list[AnalysisItemContext] = Field(default_factory=list)
    questions: list[AnalysisQuestionContext] = Field(default_factory=list)
