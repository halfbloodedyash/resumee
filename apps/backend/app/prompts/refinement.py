...
# IMPORTANT: This is a list sorted by length (longest first) to prevent
# substring replacement corruption (e.g., "paradigm shift" must be checked
# before "paradigm"). Do NOT change to a set.
_AI_PHRASE_BLACKLIST_RAW: list[str] = [
    ...
    "\u2014",  # Em-dash
    "---",
    # NOTE: "--" intentionally removed — it corrupts date ranges like "2020 -- 2023"
]

# Sorted by length (longest first) so longer phrases match before their substrings
AI_PHRASE_BLACKLIST: list[str] = sorted(_AI_PHRASE_BLACKLIST_RAW, key=len, reverse=True)
