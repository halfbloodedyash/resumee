...
class Database:
    ...
    async def create_resume_atomic_master(
        ...
        async with self._master_resume_lock:
            ...
            # Recovery behavior: if the current master is stuck in failed state,
            # promote the next upload to become the new master.
            # NOTE: We intentionally do NOT demote "processing" resumes here —
            # they are legitimately being parsed by the LLM and should complete.
    ...
    def set_master_resume(self, resume_id: str) -> bool:
        ...
        NOTE: This is a synchronous method. Callers should acquire
        ``_master_resume_lock`` externally when used in async contexts
        to prevent race conditions with ``create_resume_atomic_master``.
        """
        Resume = Query()
    ...
    async def set_master_resume_safe(self, resume_id: str) -> bool:
        """Set a resume as the master with proper locking.

        Async-safe version that acquires ``_master_resume_lock``
        to prevent race conditions.
        """
        async with self._master_resume_lock:
            return self.set_master_resume(resume_id)
