...
async def analyze_resume(resume_id: str, user_id: str = Depends(get_user_id)) -> AnalysisResponse:
...
async def generate_enhancements(request: EnhanceRequest, user_id: str = Depends(get_user_id)) -> EnhancementPreview:
    ...
    # Use the analysis context forwarded from the /analyze response
    # instead of re-running the LLM (which would produce non-deterministic IDs).
    if request.items_to_enrich and request.questions:
        # Deterministic path: use the context the frontend forwarded
        question_to_item: dict[str, str] = {
            q.question_id: q.item_id for q in request.questions
        }
        item_details: dict[str, dict] = {
            item.item_id: item.model_dump() for item in request.items_to_enrich
        }
        # Build a list-of-dicts representation of questions for downstream use
        analysis_questions: list[dict] = [
            q.model_dump() for q in request.questions
        ]
    else:
        # Legacy fallback: re-analyze (kept for backward compatibility with
        # older frontend versions that don't send context).
        resume_json = json.dumps(processed_data, indent=2)
        language = _get_content_language()
        output_language = get_language_name(language)
        ...
        analysis_questions = analysis_result.get("questions", [])
    ...
    for item_id, answers in answers_by_item.items():
        ...
        item_questions = [
            q for q in analysis_questions if q.get("item_id") == item_id
        ]
...
async def apply_enhancements(
    resume_id: str, request: ApplyEnhancementsRequest, user_id: str = Depends(get_user_id)
) -> dict:
...
async def regenerate_items(request: RegenerateRequest, user_id: str = Depends(get_user_id)) -> RegenerateResponse:
...
async def apply_regenerated_items(
    resume_id: str, regenerated_items: list[RegeneratedItem], user_id: str = Depends(get_user_id)
) -> dict:
