...
from app.utils import get_content_language as _get_content_language_util
...
def _get_content_language() -> str:
    ...
    return _get_content_language_util()
...
def _raise_improve_error(
    ...
    # Re-raise HTTPExceptions with their original status code instead of masking as 500
...
async def upload_resume(file: UploadFile = File(...), user_id: str = Depends(get_user_id)) -> ResumeUploadResponse:
...
async def get_resume(resume_id: str = Query(...), user_id: str = Depends(get_user_id)) -> ResumeFetchResponse:
...
async def list_resumes(include_master: bool = Query(False), user_id: str = Depends(get_user_id)) -> ResumeListResponse:
...
async def update_resume_endpoint(
    resume_id: str, resume_data: ResumeData, user_id: str = Depends(get_user_id)
) -> ResumeFetchResponse:
...
async def download_resume_pdf(
    ...
    template: str = Query(
        "swiss-single",
        pattern="^(swiss-single|swiss-two-column|modern|modern-two-column)$",
    ),
...
async def delete_resume(resume_id: str, user_id: str = Depends(get_user_id)) -> dict:
    """Delete a resume by ID."""
    if not db.delete_resume(resume_id, user_id):
        raise HTTPException(status_code=404, detail="Resume not found")
...
async def retry_processing(resume_id: str, user_id: str = Depends(get_user_id)) -> ResumeUploadResponse:
...
async def update_cover_letter(
    resume_id: str, request: UpdateCoverLetterRequest, user_id: str = Depends(get_user_id)
) -> dict:
...
async def update_outreach_message(
    resume_id: str, request: UpdateOutreachMessageRequest, user_id: str = Depends(get_user_id)
) -> dict:
...
async def update_title(resume_id: str, request: UpdateTitleRequest, user_id: str = Depends(get_user_id)) -> dict:
...
async def generate_cover_letter_endpoint(resume_id: str, user_id: str = Depends(get_user_id)) -> GenerateContentResponse:
...
async def generate_outreach_endpoint(resume_id: str, user_id: str = Depends(get_user_id)) -> GenerateContentResponse:
...
async def get_job_description_for_resume(resume_id: str, user_id: str = Depends(get_user_id)) -> dict:
