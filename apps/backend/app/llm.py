...
def _appears_truncated(data: dict) -> bool:
    ...
    # Only flag as truncated if MULTIPLE key sections are empty — a single
    # empty section is normal (students, career-changers, freelancers).
    suspicious_empty_arrays = ["workExperience", "education", "skills"]
    empty_count = sum(
        1 for key in suspicious_empty_arrays
        if key in data and data[key] == []
    )
    if empty_count >= 2:
        logging.warning(
            "Possible truncation detected: %d key sections are empty",
            empty_count,
        )
        return True
